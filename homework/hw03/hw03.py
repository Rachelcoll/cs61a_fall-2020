...
def balanced(m):
    ...
    if is_planet(end(left(m))) or is_planet(end(right(m))):
        if balanced(end(left(m))) or balanced(end(right(m))):
            return total_weight(end(left(m))) * length(left(m)) == total_weight(end(right(m))) * length(right(m))
    else:
...
def div_interval(x, y):
    ...
    reciprocal of y."""
    "*** YOUR CODE HERE ***"
    assert not (lower_bound(y) <= 0 and upper_bound(y) >= 0), 'cannot divide'
...
def check_par():
    ...
    r2 = interval(1, 2) # Replace this line!    -0.5,2  1,1/9
    return r1, r2
r1, r2 = check_par()
x = par1(r1, r2)
y = par2(r1, r2)
...
def quadratic(x, a, b, c):
    ...
    """
    sp_t = -b/(2*a)
    y1 = a * lower_bound(x) * lower_bound(x) + b * lower_bound(x) + c
    y2 = a * upper_bound(x) * upper_bound(x) + b * upper_bound(x) + c
    if lower_bound(x) <= sp_t <= upper_bound(x):
        if a > 0:
            lowest = a * sp_t * sp_t + b * sp_t + c
            highest = max(y1, y2)
        if a < 0:
            highest = a * sp_t * sp_t + b * sp_t + c
            lowest = min(y1, y2)
    else:
