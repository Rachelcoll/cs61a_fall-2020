...
def closer_city(lat, lon, city_a, city_b):
    ...
    if distance(city_b, city_c) <= distance(city_a, city_c):
    ...
    1
      2
        3
          ...
          2
    """
    lt = [tree(leaf) for leaf in leaves]
    if is_leaf(t):
        t = tree(label(t), lt)
...
def riffle(deck):
    ...
    for i in range(len(deck)//2):
            reconfig.append(deck[i])
            reconfig.append(deck[len(deck)//2 + i])
    return reconfig
    ...
    if is_leaf(t1) and is_leaf(t2):
        ...
        for i in range(min(len(branches(t1)), len(branches(t2))), max(len(branches(t1)), len(branches(t2)))):
            if len(branches(t1)) > len(branches(t2)):
                new_branches.append(branches(t1)[i])
            else:
                new_branches.append(branches(t2)[i])
...
def build_successors_table(tokens):
    ...
    #     prev = 
    for i in range(len(tokens)):
        if tokens[i] in table.keys():
            table[tokens[i]].append(tokens[i + 1])
        else:
            table[tokens[i]] = []
            if tokens[i] == '.':
                table[tokens[i]].append(tokens[0])
            else:
                table[tokens[i]].append(tokens[i + 1])
    return table
